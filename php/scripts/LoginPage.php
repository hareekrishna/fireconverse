...
<html>
    <head>
        ...
        <script>
			$(function(){
				...
				if(url_info['auth']=='failed') $("#error").css({'display':'block'}).find("#error_text").html("Wrong Email/Password. Try again.");
				if(url_info['auth']=='loggedOut') $("#error").css({'display':'block'}).find("#error_text").html("You have Logged-out.");
				if(url_info['auth']=='bce')$("#error").css({'display':'block','font-size':15}).find("#error_text").html("Sign-Up error, provide details as required.");	
				if(url_info['auth']=='bfa')$("#error").css({'display':'block','font-size':12}).find("#error_text").html("You have exceeded Log-in attempts.<br/> Try after sometime.");
				if(url_info['auth']=='eat')$("#error").css({'display':'block','font-size':15}).find("#error_text").html("The E-mail is already registered.");
				$("#l").click(function(){
					$("#u").css({ }).focus();
				...
				$("#s").on("click",function(){
					...
					$("#signup_inner").animate({'margin-top': '85px','height': '122px'},500,function(){
						...
						if(name_check_flag ==true && password_check_flag==true && repassword_check_flag==true && email_check_flag==true && gender_check_flag==true  ){					$("#submit_hidden").click();
    ...
    <body>
		...
		<?PHP
			...
			$s_errorMessage="";
			$error_on_password="";
			$date_of_benow= date('l jS F Y h:i:s a');
			
			include ("csign.php");
			
			if ($_SERVER['REQUEST_METHOD'] == 'POST' && ($_SESSION['resubmit_flag'] == $_POST['resubmit_flag']))
			{
				if (isset($_POST['Submit1'],$_POST['first_name'],$_POST['last_name'],$_POST['s_password'],$_POST['s_repassword'],$_POST['s_email'], $_POST['s_gender']))
				 { 
					...
					$into_db=serialize($avatars);
			 		...
			 		if($email_check=$mysqli->prepare("SELECT `email` FROM `fireconverse`.`members` WHERE email= '".$s_email."'")){
						$email_check->execute();
						$email_check->store_result();
						$r=$email_check->num_rows; 
						if($r>0){
							header('location:loginPage.php?auth=eat');
							}
						else {   
						 include("csign.php");
						 $stmt9 = $mysqli->prepare("INSERT INTO `members`(`name`, `L2`, `salt`, `date_of_benow`, `email`, `gender`, `mobileno`, `privacy`) VALUES(?,?,?,?,?,?,?,?)");  
						 if($stmt9){ 
								$stmt9->bind_param("ssssssis", $name, $password, $random_salt,$date_of_benow,$s_email,$gender,$mobileno,$privacy);   
								if($stmt9->execute()){
								  ...
								  if($stmt = $mysqli->prepare("SELECT ID,name, L2, salt FROM `fireconverse`.`members` WHERE email = '$s_email' ")){  
									  ...
									  if($db_L2 == $password){  
							 		  ...
							 		  if($stmt_memdata=$mysqli->prepare("INSERT INTO `fireconverse`.`meminfo` (ID,bday,status,city,avatars) VALUES(?,?,?,?,?) ")){
								
								 		 $stmt_memdata->bind_param('sssss',$u_ID,$bday,$status,$city,$into_db);
								  		 if($stmt_memdata->execute()) header("location:index.php");
										
										}
										else  header("location:loginPage.php?auth=bce");
					  ...
					  {  	  
						  ...
						  $L2 = hash('sha512', $L2.$salt); 
							 ...
							 if(checkbrute($u_ID, $mysqli) == true) { 
								...
								header("location:loginPage.php?auth=bfa");
					 } else {
					 if($db_L2 == $L2) {  
